...
#show: doc => article(
  ...
  authors: (
                    ( name: [Shishir Rao (https:\/\/calgaryanalyticsltd.com)],
            affiliation: [],
            location: [],
            role: [],
...
#figure([
#box(image(".\\Images/BayesianBookHoff.WEBP", width: 75.0%))
], caption: figure.caption(
position: bottom, 
[
Hoff, P. D. (2009). A first course in Bayesian statistical methods. Springer. #link("https://doi.org/10.1007/978-0-387-92407-6")
]), 
...
= Dataset
<dataset>
For the purpose of this blog, we will use the Concrete Compressive Strength dataset available on the UC Irvine Machine Learning Repository website. The full dataset can be found #link("https://archive.ics.uci.edu/ml/datasets/concrete+compressive+strength")[here];#footnote[Yeh, I. (1998). Concrete Compressive Strength. UCI Machine Learning Repository. https:\/\/doi.org/10.24432/C5PK67.];.
...
A subset of the dataset with only the relevant columns for this analysis has been extracted. I have artificially introduced some missingness in the dataset to illustrate the imputation methods. Note that the missingness is Missing at Random (MAR). This means that the missingness is independent of the sample (and the parameters of the Multivariate Normal model that we will fit).
