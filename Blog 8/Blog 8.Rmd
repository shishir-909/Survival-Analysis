...
Introductory books on statistics include formulas to calculate the required sample size when estimating the mean of a normal distribution for complete data i.e no censoring. In reliability applications, we usually have censored data, and the distributions are not necessarily normal.^[Typical lifetime distributions are Weibull and log-normal.] 
...
Suppose a manufacturer of mechanical seals has developed a new design and wants to conduct a life test for the new seal. The aim is to estimate the 5% quantile ^[The time by which 5% of the population of the newly designed seals will fail]. The manufacturer will accept a precision factor of 2. The maximum duration that the manufacturer is willing to test the seal is 4000 hours^[This is a very long test and does not happen in practice. Usually, life tests are accelerated by subjecting the newly designed product to conditions harsher than what would be expected on the field, which helps in reducing the duration of the life test. For this blog, we assume that there is no acceleration and that the manufacturer is willing to test the product in conditions similar to actual conditions]. 

What does the precision factor 2 actually represent?

To answer this question, we will start with the formula for the 100(1-$\alpha$)% confidence interval for $log(t_{p})$, where $t_{p}$ is the $p^{th}$ quantile of the time to failure distribution^[$log(t_{p}) = \mu + \Phi^{-1}(p)\sigma$ in the log-location scale representation].
...
$$
where $t_{c}$ is the censoring time, which is the duration of the test. The manufacturer can test to a maximum of 4000 hours, so $t_{c}$ is 4000 hours. $\mu$ and $\sigma$ are the planning values and $\Phi_{SEV}$ is the CDF of the standard smallest extreme value distribution.  
...
2. All analyses were performed using R Statistical Software(v4.4.1; R Core Team 2024). Please refer to next section for all the packages used, their versions and the names of the package developers who deserve credit for building these amazing open source packages.
