...
The following article demonstrates the application of parametric regression modeling to data from a life test of ceramic ball bearings and is from a paper by McCool (1980)^[McCool, J. I. (1980). Confidence limits for Weibull regression with censored data. *IEEE Transactions on Reliability 29*, 145–150.]. The methods that I employ in this analysis are from  another excellent book^[Statistical Methods for Reliability Data, Second Edition (William Q. Meeker, Luis A. Escobar, Francis G. Pascual)] that I have been referring to recently and came across the ceramic data set in the same book. The data set is also publicly available on DataShare, Iowa State University's open data repository which can be accessed through [**this link**](https://iastate.figshare.com/articles/dataset/Reliability_Data_Laboratory_Failure-Time_and_Single-Time_Strength_Data/14454753?backTo=%2Fcollections%2FData_Sets_Used_in_Statistical_Methods_for_Reliability_Data_Second_Edition_%2F5395665&file=28330266). 

The R statistical programming language is used for all computations and plots below. Unlike my previous articles, I have not included any R code within the article, but they are available on my Github page and can be accessed [**here**](https://github.com/shishir-909/Survival-Analysis/blob/d10f2b18c2eaf745f1188e368d3c7f882efc34ca/Blog%205/Blog%205.Rmd). The code for some of the plots are lengthy and I wanted to avoid code chunks taking up space on this page. 
...
```{r Std Resid Weibull plot, echo=FALSE, fig.cap="Figure 8: Weibull Probability Plot of Standardized Residuals", cache=TRUE}
...
```{r Weibull BoxCox, eval=FALSE, include=FALSE}
...
chisq_statistic <- 2*(para_fit_weibull.Factor[["loglik"]][2] - para_fit_Weibull.lambda[["loglik"]][2])

1-pchisq(chisq_statistic,2)

```
Box cox transformation didn't help with Weibull.

Try log-normal. First, plot LogNormal EqualSig

```{r Log normal factor}
para_fit_logNormal.Factor <- survival::survreg(Surv(Revolutions, delta) ~ factor(Stress, levels = uniqueStress), data = ceramic, dist = "lognormal")
...
```{r Prep ML table 2 logNormal, include=FALSE}
Fig3.paras <- data.frame(
  mu = c(
    Fig17.a_MLest[["0.87"]][1],
    Fig17.a_MLest[["0.99"]][1],
    Fig17.a_MLest[["1.09"]][1],
    Fig17.a_MLest[["1.18"]][1]
  ),
  se.mu = c(
    Fig17.a_MLest[["0.87"]][4],
    Fig17.a_MLest[["0.99"]][4],
    Fig17.a_MLest[["1.09"]][4],
    Fig17.a_MLest[["1.18"]][4]
  ),
  sigma = c(
    Fig17.a_MLest[["0.87"]][2],
    Fig17.a_MLest[["0.99"]][2],
    Fig17.a_MLest[["1.09"]][2],
    Fig17.a_MLest[["1.18"]][2]
  ),
  se.sigma = c(
    Fig17.a_MLest[["0.87"]][5],
    Fig17.a_MLest[["0.99"]][5],
    Fig17.a_MLest[["1.09"]][5],
    Fig17.a_MLest[["1.18"]][5]
  )
)

#knitr::kable(Fig3.paras, "latex")

rownames(Fig3.paras) <- c("0.87 Mpsi", "0.99 Mpsi", "1.09 Mpsi", "1.18 Mpsi")
colnames(Fig3.paras) <- c("$\\hat{\\mu}$","$\\mathrm SE_{\\hat{\\mu}}$","$\\hat{\\sigma}$","$\\mathrm SE_{\\hat{\\sigma}}$")

SepDists_LL_logNormal <- round(Fig17.a_MLest[["0.87"]][3] + Fig17.a_MLest[["0.99"]][3] + Fig17.a_MLest[["1.09"]][3] + Fig17.a_MLest[["1.18"]][3],3)

chisq_statistic <- 2*(SepDists_LL_logNormal - para_fit_logNormal.Factor[["loglik"]][2])

1-pchisq(chisq_statistic,3)

```

Again, no evidence of difference between log-normal SepDist and and log-normal EqualSig.

First plot log-normal EqualSig

```{r Log normal EqualSig prep}
mu.0.87.Fig17.7c <- para_fit_logNormal.Factor[["coefficients"]][1] #baseline/intercept

se.mu.0.87.Fig17.7c <- sqrt(para_fit_logNormal.Factor[["var"]][1,1])

mu.0.99.Fig17.7c <- mu.0.87.Fig17.7c + para_fit_logNormal.Factor[["coefficients"]][2] #Because mu.270.Fig17.7c is the intercept i.e baseline

se.mu.0.99.Fig17.7c <- sqrt(para_fit_logNormal.Factor[["var"]][1,1] + para_fit_logNormal.Factor[["var"]][2,2] + (2*para_fit_logNormal.Factor[["var"]][1,2])) #Var(beta.0 + beta.1) = Var(beta.0) + Var(beta.1) + 2*Covar(beta.0. beta.1)

mu.1.09.Fig17.7c <- mu.0.87.Fig17.7c + para_fit_logNormal.Factor[["coefficients"]][3] #Because mu.270.Fig17.7c is the intercept i.e baseline

se.mu.1.09.Fig17.7c <- sqrt(para_fit_logNormal.Factor[["var"]][1,1] + para_fit_logNormal.Factor[["var"]][3,3] + (2*para_fit_logNormal.Factor[["var"]][1,3])) #Var(beta.0 + beta.1) = Var(beta.0) + Var(beta.1) + 2*Covar(beta.0. beta.1)

mu.1.18.Fig17.7c <- mu.0.87.Fig17.7c + para_fit_logNormal.Factor[["coefficients"]][4] #Because mu.270.Fig17.7c is the intercept i.e baseline

se.mu.1.18.Fig17.7c <- sqrt(para_fit_logNormal.Factor[["var"]][1,1] + para_fit_logNormal.Factor[["var"]][4,4] + (2*para_fit_logNormal.Factor[["var"]][1,4])) #Var(beta.0 + beta.1) = Var(beta.0) + Var(beta.1) + 2*Covar(beta.0. beta.1)

sigma.Fig17.7c <- para_fit_logNormal.Factor[["scale"]]

se.sigma.Fig17.7c <- sqrt(para_fit_logNormal.Factor[["var"]][4,4] * (para_fit_logNormal.Factor[["scale"]]**2))
```


```{r LogNormal plot EqualSig, echo=FALSE, fig.cap="Figure 5: LogNormal plot with the shape parameter constrained to be equal across all stress levels", cache=TRUE}
#Log-Normal plot 17.7 c
...
ggplot() + geom_point(
  ...
  aes(intercept = (-mu.1.18.Fig17.10c/sigma.Fig17.10c), #y-axis intercept
      slope = 1/sigma.Fig17.10c, color = "1.18 Mpsi")
) + ggtitle(label = "LogNormal EqualSig") + scale_color_manual(name="Stress",values = cols)


```
Looks bad, especially at the 0.87 Mpsi stress level.

Still, lets try a logNormal regression model with log(Stress) as the explanatory variable.


```{r Regression model LogNormal, include=FALSE}
para_fit_logNormal.Fig17.7d <- survival::survreg(Surv(Revolutions, delta) ~ log(Stress), data = ceramic, dist = "lognormal")

summary(para_fit_logNormal.Fig17.7d)

chisq_statistic <- 2*(para_fit_logNormal.Factor[["loglik"]][2] - para_fit_logNormal.Fig17.7d[["loglik"]][2])

1-pchisq(chisq_statistic,2)
```
Similar to weibull regression, we found evidence that there is a difference between the logNormal regression model and logNormal EqualSig.

```{r Regression model LogNormal Table, include=FALSE}
beta.0 <- para_fit_logNormal.Fig17.7d[["coefficients"]][1]

se.beta.0 <- sqrt(para_fit_logNormal.Fig17.7d[["var"]][1,1])

beta.1 <- para_fit_logNormal.Fig17.7d[["coefficients"]][2]

se.beta.1 <- sqrt(para_fit_logNormal.Fig17.7d[["var"]][2,2])

sigma.Fig17.7d <- para_fit_logNormal.Fig17.7d[["scale"]]

se.sigma.Fig17.7d <- sqrt(para_fit_logNormal.Fig17.7d[["var"]][3,3] * (para_fit_logNormal.Fig17.7d[["scale"]]**2))

RegrParas <- data.frame(
  ML_est = c(beta.0, beta.1, sigma.Fig17.7d),
  Std.Err = c(se.beta.0, se.beta.1, se.sigma.Fig17.7d),
  Wald_0.95_Lower = c(
    beta.0 - (qnorm(0.975) * se.beta.0),
    beta.1 - (qnorm(0.975) * se.beta.1),
    exp(log(para_fit_logNormal.Fig17.7d[["scale"]]) - (
      qnorm(0.975) * sqrt(para_fit_logNormal.Fig17.7d[["var"]][3, 3])
    ))
  ),
  Wald_0.95_Upper = c(
    beta.0 + (qnorm(0.975) * se.beta.0),
...
Regr_LL <- round(para_fit_logNormal.Fig17.7d[["loglik"]][2],3)

```

Try boxCox with logNormal.
...
while(i <= length(lambda.seq)){
  ...
  para_fit_logNormal.lambda <- survival::survreg(Surv(Revolutions, delta) ~ Stress.boxCox, data = ceramic, dist = "lognormal")

lambda.log.likelihood$LL[i] <-  para_fit_logNormal.lambda[["loglik"]][2]
...
chisq_statistic <- 2*(para_fit_logNormal.Factor[["loglik"]][2] - para_fit_logNormal.lambda[["loglik"]][2])

1-pchisq(chisq_statistic,2)
```
BoxCox didn't help. Evidence of a difference between bocCox logNormal regression and logNormal EqualSig.

I am left with no choice but to try the non-constant sigma model. I will try the non-constant sigma model on the Weibull distribution since the log likelihoods were higher so far, as compared to the logNormal models.


```{r IRLS computations continuous varSig}
...
Now that I have found the ML estimates using IRLS, find out the log likelihood of this new model.
...
The LR test is still significant. Can't do much about it. We will stick to the varying sigma model. 

Plot Weibull plot of varying sigma Weibull model.
...
```{r Weibull plot varSig Regression, echo=FALSE, fig.cap="Figure 6: Weibull plot of the varSig regression model", cache=TRUE}
#Weibull plot 17.10 d
...
The varSig regression Weibull plot isn't much better than the regression Weibull plot :(

I will try fitting a value of sigma before 0.9 and a different value above 0.9


```{r IRLS computations discrete varSig}

ceramic <- ceramic %>%
  dplyr::mutate(logStress = log(Stress)) %>%
  dplyr::mutate(y = log(Revolutions)) %>%
  dplyr::mutate(x.s = case_when(Stress < 1 ~ 0,
                                Stress > 1 ~ 1))
...
iterest <- function(beta.0, beta.1, beta.0s, beta.1s){
  ...
  n <- nrow(X.matrix)
  n.F <- sum(ceramic$delta)
  sigma.i <- exp(beta.0s + (beta.1s*ceramic$x.s))
  ...
  ceramic.F <- ceramic.1 %>%
    dplyr::filter(delta==1) %>%
    dplyr::rowwise() %>%
    dplyr::mutate(Fl = ((y - (c(1,logStress) %*% beta.vector.star))**2)*((exp(r.i)-1)/r.i)/(exp(2*beta.1s*x.s)))
  ...
  beta.1sfn <- function(beta.1s){
    sum(ceramic.F1$Fl/exp(2*beta.1s*ceramic.F1$x.s)) - sum(ceramic.F1$logStress)
    # + sum(superAlloy.C1$Ce/exp(2*beta.1s*superAlloy.C1$logStress)) 
  }
...
```{r}
sigma.i_final <- exp(beta.0s + (beta.1s*ceramic$x.s))
r.i_final <- (ceramic$y-(beta.0 + (beta.1*ceramic$logStress)))/sigma.i_final
...
sigma.0.87.varSig <- exp(beta.0s + (beta.1s*0))

mu.0.99.varSig <- beta.0 + (beta.1*log(0.99))

sigma.0.99.varSig <- exp(beta.0s + (beta.1s*0))

mu.1.09.varSig <- beta.0 + (beta.1*log(1.09))

sigma.1.09.varSig <- exp(beta.0s + (beta.1s*1))

mu.1.18.varSig <- beta.0 + (beta.1*log(1.18))

sigma.1.18.varSig <- exp(beta.0s + (beta.1s*1))
...
Try removing the outlier observation and then see what you get.
