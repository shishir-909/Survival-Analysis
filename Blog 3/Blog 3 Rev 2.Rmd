---
title: "Hypothesis Testing in Failure Time Data: Part I"
...
I recently read the chapter on non parametric hypothesis testing of survival data from the book Survival Analysis: Techniques for Censored and Truncated Data, Second Edition (John P. Klein and Melvin L. Moescheberger), which is mainly written from a biology and medical applications standpoint. This article is my attempt at applying the concepts I learnt in that chapter to time-to-failure data from reliability. 

The methods discussed in this article deal with testing for difference in failure rates between groups against the null hypothesis of no difference between groups. The intent is to check whether there is a practical and statistical difference between the failure rates of the different groups. One would want to make such comparisons for various reasons. For example, suppose we have failure data of two components from different vendors and we want to find out which component has a lower failure rate. Another reason could be to check for any differences in failure rates before and after a process improvement change has been implemented, or between components operating on different mine sites or different conditions. These methods would be applicable to all such datasets that are right censored (or left truncated and right censored).

Two datasets^[From the book Statistical Methods for Reliability Data, Second Edition (William Q. Meeker, Luis A. Escobar, Francis G. Pascual)] will be analysed. Each dataset consists of data from at least two groups. All datasets are available on the student companion website of this book. They are also publicly available on DataShare, Iowa State University's open data repository which can be accessed through [**this link**](https://doi.org/10.25380/iastate.c.5395665). 

Note that I have only used non-parametric hypothesis testing methods in this article. Non-parametric methods do not make any assumptions for the distribution of failure times. These methods are normally tried first before moving on to parametric models, which, if the assumptions are met, could lead to more precise conclusions. 

This blog article discusses the application of the non-parametric log rank test. Other non-parametric tests like test for trend, stratified tests etc will be discussed in subsequent blogs.
...
\\
\\
&H_A: h_{old}(t) \, \text{is different to} \, h_{new}(t) \, \text{for some} \,  t \leq \tau. \quad \text{(A)}
\end{aligned}
$$
...
I also calculated the log rank test statistic and the p-value manually and was able to get a similar result^[R code for manual calculations can be found in the R markdown file for this article, available in my Github repository linked in the introduction].
...
It gives us an incorrect p-value of 0.7. Hence, we should not use this functionality from *ggsurvplot*. Here is the correct plot with the p-value hardcoded.
...
These results also match with my manual calculations^[Can be found in my github repository].
...
It is also important to verify our results to make sure that the answer we get is indeed correct. Open source software like R has made data analysis universally accessible, but one must also be careful and avoid blindly applying packages without verifying results. Mature packages like *survival*, which is regularly maintained and has been evolving for close to 4 decades is an excellent tool for survival analysis. But when it comes to newer packages which are still evolving, or packages that are not regularly maintained, it is best to verify before using it in any production environment. 
