...
There are times when we want to compare time-to-failure data from two or more groups. For example, a designer might want to implement a new design to reduce costs, but wants to make sure that this change does not adversely affect the reliability of the product. Or, a manufacturer might want to find out if there is any difference in the time-to-failure of products manufactured from different batches or different assembly lines. Parametric and non-parametric methods to compare failure time distributions are widely used for such purposes. Two of my previous blog articles([**here**](https://rpubs.com/shishir909/1199328) and [**here**](https://rpubs.com/shishir909/1226309)) dealt with non-parametric methods for comparing failure data. In this article, I demonstrate the use of a parametric method for the same purpose.

I have not included any R code within the article, but they are available on my Github page and can be accessed [**here**](https://github.com/shishir-909/Survival-Analysis/blob/931301ffee99b54144d53f63936ae3a09abe5aa1/Blog%206/Blog%206_final.Rmd).

# Data

The data used in this article is from the book Statistical Methods for Reliability Data, Second Edition (William Q. Meeker, Luis A. Escobar, Francis G. Pascual)]. Table 1 shows this data, and it is also publicly available on DataShare, Iowa State University's open data repository which can be accessed through [**this link**](https://iastate.figshare.com/articles/dataset/Reliability_Data_Field_Failure-time_Data/14454756?backTo=%2Fcollections%2FData_Sets_Used_in_Statistical_Methods_for_Reliability_Data_Second_Edition_%2F5395665&file=28330629). 

The values in Table 1 are the results of a pull test to measure the bond strength of wire bonds from three separate manufacturing batches. We are interested in finding out whether there are statistically significant differences in the distribution of bond strength across the different manufacturing batches. If we do find that there is a statistically significant differnce, we want to find out whre is this difference (between which pairs of batches).
...
Note that 3 **separate** Weibull distributions were fit - one for each batch. Similarly, 3 **separate** log normal distributions were also fit. Looking at both the plots above, the Weibull distribution seems to fit slightly better than the log-normal, but not by much. The data could have come from any of the two distributions we have considered here. We are mainly looking for any indication that a particular distribution is *definitely* not a good fit rather than *proving* that the data is from a particular distribution^[This was also mentioned in my previous article on [**parametric regression**](https://rpubs.com/shishir909/1251052)]. The AIC from the Weibull model is `r AIC_Weibull` and the log-normal model is `r AIC_Lognormal`. Since the AIC is lower for the Weibull model, we will continue with Weibull distribution, although log-normal wouldn't have been a terrible choice either.

The log location scale model for the Weibull distribution is shown in Eq. A below^[Although we have used the letter "T", which usually denotes "time to failure", this would denote the strength variable in our current example.].
...
Table 2 shows the location ($\mu$) and scale ($\sigma$) parameters from the log-location scale representation of the Weibull distribution.  
...
ML_estimates.Weibull <- ML_estimates.Weibull %>%
  dplyr::mutate(Parameter = c("$\\hat{\\mu_{Batch1}}$","$\\hat{\\sigma_{Batch1}}$","$\\hat{\\mu_{Batch1}}$","$\\hat{\\sigma_{Batch1}}$","$\\hat{\\mu_{Batch1}}$","$\\hat{\\sigma_{Batch1}}$")) %>%
...
Figure 3 shows the approximate 90% joint confidence regions based on the relative likelihood for the Weibull distribution median and the shape parameter $\beta$^[$\beta = 1/\sigma$]. There is a significant amount of overlap between Batches 4 and 5, which suggests that the difference in medians between these two batches might not be statistically significant. We can also see in Table 2 that the confidence intervals for the location parameter $\mu$ for Batches 4 and 5 overlap. The amount of overlap for Batch 1 with Batch 5 is very low, which means the difference in medians of Batch 1 and Batch 5 might be statistically significant. The size of the region of overlap between Batch 1 and 4 is lower than the size of overlap between Batch 4 and 5 and higher than the size of overlap between Batch 1 and 5.     

```{r Joint Confidence Region, echo=FALSE, fig.cap="Figure 3: 90% Relative Likelihood Contour plot"}
#Contour plot
...
```{r Median difference Bonferroni, include=FALSE, tab.cap="Table 3: Estimates of differences in medians"}
...
From Table 3, we see that it is only the simultaneous confidence interval for $\Delta_{51}$ that does not contain zero, which means we have evidence at the 10% significance level to say that the medians for Batches 1 and 5 are different. Since the CI is negative, we can say that we have evidence that the median for Batch 1 is greater than the median of Batch 5 (at the 10% significance level). We cannot say the same for the other two comparisons since their CIs includes zero. 


# Analysis with Equal Shape ($\sigma$) parameter

In the previous section, we fit three separate Weibull distributions - one for each Batch. This led to the estimation of 6 parameters ($\mu_{Batch1}, \mu_{Batch4},\mu_{Batch5},\sigma_{Batch1},\sigma_{Batch4},\sigma_{Batch5}$), as shown in Table 2. When comparing groups where the mode of failure is the same in all groups, it is reasonable to try fitting a distribution that has the same $\sigma$ across all groups and only let the location i.e $\mu$ vary across the groups. Such a model can be represented by the following regression equation:
...
\end{align*}
$$
Notice that in this model, we have the same $\sigma$ parameter for all batches. The advantage of using such a model is that it uses the available data to fit only 4 parameters ($\beta_{0}$, $\beta_{1}$, $\beta_{2}$ and $\sigma$) instead of 6 parameters that we fit in the last section^[Lower the number of parameters to be estimated, simpler is the model and lower is the risk of overfitting a model.].

Table 4 below shows the estimate, standard error and the Wald 90% confidence interval for the parameters from Eq. B. Figure 4 shows the Weibull probability plot for this model. The $\sigma$ parameter determines the slope of the line in the Weibull plot. Since it is the same for all three batches, the lines in this plot are parallel to each other as opposed to the lines in Figure 1. 
...
```{r Weibull Probability Plot EqualSig, echo=FALSE}
...
We can conduct pairwise comparisons using the equal $\sigma$ model. In the previous section, we conducted such comparisons at the median (50% quantile). In the equal $\sigma$ model, we are not limited to any particular quantile. Since the lines in the Weibull plot are parallel, the difference between the batches at any quantile is going to be the same.

Table 5 below shows the estimate of the difference between any two batches along with the standard error and simultaneous confidence intervals. The simulataneous confidence intervals for the difference between batches 1 and 4 and batches 1 and 5, both do no contain zero. Thus, we have evidence that batches 1 and 4 are different and batches 1 and 5 are different at the 10% significance level. We cannot say the same for the comparison between batches 4 and 5. 


```{r Any quantile difference Bonferroni}
...
A likelihood ratio test comparing the model in the previous section (separate distributions) with the model in this section (equal $\sigma$) gives a p-value of 0.609, which indicates that we do not have enough evidence to distinguish between the models. Hence, we can choose the model with the lower number of parameters i.e the one with equal $\sigma$. 

A pairwise comparison with this model is not restricted to any particular quantile. 

We can conduct a likelihood ratio test to compare the model in the previous section and the model with the same $\sigma$. The p-value for this test is  

$$
\chi^{2}_{LR} = 2(LogLikelihood_{Seperate} - LogLikelihood_{EqualSigma})\\
p-val = \chi_{LR}
$$


We can continue with the pairwise comparisons of the three batches. There is a difference between this comparison and the previous comparison though. Since the $\sigma$ 

When constructing simultaneous confidence intervals, it is important to 



the distributions do not appear to be differnt at the 10% significance level. is relatively smaller   of the Weibull parameters. for the median of the distributions in Figure 3 also show a large amount of overlap between Batches 4 and 5.  an overlap between Batches 4 and 5. Thus, we do not have sufficient evidence suggesting that Batch 4 is different to Batch 5.   
...
The table shows that there is an overlap between the confidence interval for the location parameter.  location estimate confidence interval for Batch 4 and 

The Weibull model we fit above has 2 parameters for each batch, so there are 6 parameters to be estimated in total. In some situations, when the failure mode is the same across all the groups, it is worth checking if the 
...
Checking if bond strength is a good dataset for the blog.
...
Plot weibull and log normal plots, with separate distributions for each batch.
...
ggplot() + geom_point(data = CDF_Batch1.df, aes(x = Strength, y = p), col = "red") + geom_abline(
  aes(intercept = (-mu.ML_Batch1.Weibull/sigma.ML_Batch1.Weibull), #y-axis intercept
      slope = 1/sigma.ML_Batch1.Weibull),
      col = "red"
) + geom_point(data = CDF_Batch4.df, aes(x = Strength, y = p), col = "blue") + geom_abline(
  aes(intercept = (-mu.ML_Batch4.Weibull/sigma.ML_Batch4.Weibull), #y-axis intercept
      slope = 1/sigma.ML_Batch4.Weibull),
      col = "blue"
) + geom_point(data = CDF_Batch5.df, aes(x = Strength, y = p), col = "green") + geom_abline(
  aes(intercept = (-mu.ML_Batch5.Weibull/sigma.ML_Batch5.Weibull), #y-axis intercept
      slope = 1/sigma.ML_Batch5.Weibull),
      col = "green"
) + scale_x_continuous(
  transform = xTransformer.weibull,
...
Weibull looks a better fit

Compare AIC

```{r eval=FALSE, include=FALSE}
(AIC(fit_2.Batch1.Weibull) + AIC(fit_2.Batch4.Weibull) + AIC(fit_2.Batch5.Weibull))

(AIC(fit_2.Batch1.logNormal) + AIC(fit_2.Batch4.logNormal) + AIC(fit_2.Batch5.logNormal))

ll_SepDists.weibull <- fit_2.Batch1.Weibull$loglik[2] + fit_2.Batch4.Weibull$loglik[2] + fit_2.Batch5.Weibull$loglik[2]

```

Again, Weibull looks like a better fit. But note that Weibull is better only for batch 1. For batches 4 and 5, log normal appears a better fit.

Continue with Weibull fit since batch 1 has the highest number of failures.

Next, build an EqualSig models and conduct an LR test with SepDist model.
...
1-pchisq(chi.sq.Stat, 2) #p-val = 0.609

fit_4 <- survival::survreg(Surv(Strength, delta) ~ 1, data = bondStrength, dist = "weibull")

chi.sq.Stat.2 <- 2*(fit_3$loglik[2] - fit_4$loglik[2])

1-pchisq(chi.sq.Stat.2,2) #p-val = 0.0043

```

No reason to believe that sigma is different, but there is reason to believe that location parameter is different.

Now, construct simultaneous hypothesis test by constructing CI for the difference in median - for the SepDists and EqualSig model.
...
Only the pairwise comparison of Batch 1 vs Batch 5 does not contain 0. So, from the data, I can only say that there is evidence that Batch 1 is different to Batch 5. That is it. 

Now, do the same comparison for the EqualSig model.
...
Here, there is evidence that Batch1 vs Batch4 are different, Batch1 vs Batch5 are different, but there is no evidence that Batch4 and Batch5 are different. 

Although the difference is not statistically significant, we are more concerned with practical significance of the differences. Looking at the contour plot in Fig 3, there is a significant amount of overlap between Batches 4 and 5, which again suggests that the difference between these two batches might not be statistically significant. This is also evident in Table 2, where we see that the confidence intervals for the location parameter $\mu$ for Batches 4 and 5 overlap. On the other hand, the amount of overlap between Batch 1 and the other two batches is significantly less, suggesting that the difference could be statistically significant. 

We are able to conduct pairwise comparisons between the batches at a particular quantile to get additional insight. Since the confidence regions have been constructed using the median (50% quantile), we will use the same for the pairwise comparisons. Constructing simultaneous confidence intervals for the difference in log(medians)^[Why log of median and not just median? Because, we will use the log-location scale representation of the Weibull distribution as shown in Equation A above and it is easier to work with the log median. The conclusion is not affected by this transformation.] of two groups is a good way of making such comparisons. If the interval contains zero, we do not have evidence that the medians are different for that pair. If the interval does not contain zero, then we do have evidence of a difference in the medians for that pair. That is, we want to construct the simultaneous confidence intervals for the following:
